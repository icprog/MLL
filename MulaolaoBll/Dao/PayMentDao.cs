    ...
    public class PayMentDao
    {
        ...
        public DataTable GetDataTablePqo ( string numList ,string idxList )
        {
            ...
            */
            ...
            strSql . Append ( "SELECT JM90,JM09,JM93,JM14,JM107,CONVERT( DECIMAL(18,2),SUM(JM)) JM,CASE WHEN ISNULL(AK011,0)<CONVERT( DECIMAL(18,2),SUM(JM)) THEN ISNULL(AK011,0) ELSE CONVERT( DECIMAL(18,2),SUM(JM)) END AK,A.AK017 FROM (SELECT A.idx,JM90,CASE WHEN JM09 LIKE '%密度板%' THEN '密度板' WHEN JM09 LIKE '%胶合板%' OR JM09 LIKE '%曲板%' THEN '胶合板' ELSE '胶合板' END JM09,JM93,JM14,CASE WHEN JM107='F' OR JM107 IS NULL THEN 'F' WHEN JM107='T' THEN 'T' END JM107,CASE WHEN JM10 = 0 THEN 0 WHEN JM10 != 0 THEN JM103/JM10* JM11 END JM,JM01,SUM(ISNULL(AK011,0)) AK011,AK017 FROM R_PQO A LEFT JOIN R_PQAL B ON A.JM01=B.AL002 AND A.idx=B.AL003 LEFT JOIN R_PQAK C ON B.AL001=C.idx INNER JOIN R_REVIEWS D ON A.JM01=D.RES06 " );
            strSql . Append ( "WHERE JM90!=''  AND C.idx IN (" + idxList + ") AND RES05='执行' AND JM90 IN (" + numList + ") AND (JM90!='' AND JM90 IS NOT NULL) GROUP BY A.idx,JM90,JM09,JM14,JM93,JM107,JM10,JM11,JM01,JM103,AK017 " );
