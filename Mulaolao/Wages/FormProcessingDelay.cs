...
{
    public partial class FormProcessingDelay :FormChild
    {

        MulaolaoBll.Bll.ProcessingDelayBll _bll=null;
        string strWhere=" AND 1=1";
        DataTable tableView;

        public FormProcessingDelay ( )
        {
            InitializeComponent ( );

            _bll = new MulaolaoBll . Bll . ProcessingDelayBll ( );
        ...
        protected override void select ( )
        {
            ...
            if ( !string . IsNullOrEmpty ( dtStart . Text ) && !string . IsNullOrEmpty ( dtEnd . Text ) )
                strWhere = strWhere + string . Format ( " AND D.RES04 BETWEEN '{0}' AND '{1}'" ,dtStart . Text ,dtEnd . Text );
